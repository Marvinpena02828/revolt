		...
		<head>
			...
			<style>
				* { margin: 0; padding: 0; box-sizing: border-box; }
				...
				.servers-list { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 15px; }
				.server-item { background: #333; padding: 15px; border-radius: 5px; border-left: 4px solid #4CAF50; }
				.server-item h3 { margin-bottom: 10px; }
				.status { padding: 8px 12px; border-radius: 3px; font-size: 12px; font-weight: bold; }
				.status.running { background: #4CAF50; }
				.status.stopped { background: #f44336; }
				button { background: #4CAF50; color: white; border: none; padding: 10px 20px; border-radius: 4px; cursor: pointer; margin-top: 10px; }
				button:hover { background: #45a049; }
				.port-info { color: #aaa; font-size: 12px; margin-top: 8px; }
			</style>
		</head>
		<body>
			<div class="container">
			...
			<script>
				...
				socket.on('servers', (servers) => {
					...
					if (!servers || servers.length === 0) {
						list.innerHTML = '<p>No servers running yet</p>';
						return;
					}
					...
					list.innerHTML = servers.map(server => \`
						<div class="server-item">
							...
							<div class="status \${server.is_running ? 'running' : 'stopped'}">
								\${server.is_running ? '🟢 Running' : '🔴 Stopped'}
							</div>
							<div class="port-info">
								Port: \${server.port || 'N/A'}<br>
								Mode: \${server.is_headless ? 'Headless' : 'Headful'}
							</div>
							<button onclick="openServer('\${server.port}')">Open Dashboard</button>
							<button onclick="deleteServer('\${server.folder}')" style="background: #f44336;">Delete</button>
						</div>
					\`).join('');
				});

				function addNewServer() {
					const slug = 'server-' + Math.random().toString(36).substr(2, 9);
					fetch('/api/add_server', { method: 'POST' })
						.then(() => alert('Bot instance starting...'))
						.catch(err => alert('Error: ' + err));
				}

				function openServer(port) {
					if (!port) {
						alert('Server not running yet');
						return;
					}
